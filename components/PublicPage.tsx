...
const PublicPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const results = config.news.filter(item =>
      item.status !== 'draft' && (
        ...
        stripHtml(item.content).toLowerCase().includes(lowerQuery) ||
  ...
  // Filtered News Logic
  const featuredNews = config.news.filter(n => n.isFeatured && n.status !== 'draft');
  const regularNews = config.news.filter(n => !n.isFeatured && n.status !== 'draft').sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
                ...
                {searchResults.length > 0 ? (
                  searchResults.map(result => (
                    <div key={result.id} onClick={() => { navigateToNews(result); setSearchQuery(''); }} className="p-3 hover:bg-gray-50 cursor-pointer border-b last:border-0">
                      ...
                      <p className="text-xs text-gray-500 line-clamp-1">{stripHtml(result.content)}</p>
                    </div>
                  ))
                ) : (
